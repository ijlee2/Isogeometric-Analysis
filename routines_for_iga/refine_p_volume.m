...
%  Summary:
%  
%  This routine does a p-refinement (degree elevation) of the given
%  knot vector in 3D.
%  
%  
%  Warning:
...
%  
%      [knots1_new, knots2_new, knots3_new, nodes_new, p1_new, p2_new, p3_new] = refine_p_volume(knots1, knots2, knots3, nodes, p1, p2, p3, q1, q2, q3);
%  
%  where,
%  
...
%      q1, q2, q3 are the degree increments
%  
%  
%  Outputs:
%  
...
%  3. New degrees
%--------------------------------------------------------------------------
function [knots1_new, knots2_new, knots3_new, nodes_new, p1_new, p2_new, p3_new] = refine_p_volume(knots1, knots2, knots3, nodes, p1, p2, p3, q1, q2, q3)
    ...
    elseif (q1 == 0 && q2 == 0 && q3 == 0)
        fprintf('Warning: Please specify a positive number for q1, q2, or q3 for degree elevation.\n\nNo operation has been performed.\n\n');
    ...
    if (q1 > 0)
        ...
        shifts_for_bezierNodes     = repmat(constant_p1p1     * temp2, numNodes3, 1) + kron((constant_p1p1     * numNodes2) * temp3, ones2);
        shifts_for_bezierNodes_new = repmat(constant_p1pq1_p1 * temp2, numNodes3, 1) + kron((constant_p1pq1_p1 * numNodes2) * temp3, ones2);
        ...
        %------------------------------------------------------------------
        %  Set variables for the Bezier element [xi1, xi2] x knots2 x knots3
        %------------------------------------------------------------------
        % Set the Bezier nodes array to that for the first Bezier element
        bezierNodes = nodes(repmat((1 : constant_p1p1)', numNodes2 * numNodes3, 1) + kron(shifts_for_nodes, ones(constant_p1p1, 1)), :);
        ...
        bezierNodes_new = zeros(constant_p1pq1_p1 * numNodes2 * numNodes3, numDimensions);
        
        % Bezier nodes shared by the next Bezier element
        bezierNodes_nextElement = zeros((p1 - 1) * numNodes2 * numNodes3, numDimensions);
    ...
    if (q2 > 0)
        ...
        shifts_for_bezierNodes     = repmat(temp1, numNodes3, 1) + kron((numNodes1_new * constant_p2p1    ) * temp3, ones1) - (numNodes1_new - 1);
        shifts_for_bezierNodes_new = repmat(temp1, numNodes3, 1) + kron((numNodes1_new * constant_p2pq2_p1) * temp3, ones1) - (numNodes1_new - 1);
        ...
        nodes_new = zeros(numNodes1_new * numNodes2_new * numNodes3, numDimensions);
        nodes_new(numNodes1_new + shifts_for_nodes_new, :) = nodes(numNodes1_new + shifts_for_nodes, :);
        index_node = 2;
        
        
        %------------------------------------------------------------------
        %  Set variables for the Bezier element knots1 x [xi1, xi2] x knots3
        %------------------------------------------------------------------
        % Set the Bezier nodes array to that for the first Bezier element
        bezierNodes = nodes((1 : (numNodes1_new * constant_p2p1 * numNodes3))', :);
        
        % Initialize the coefficients for knot insertion
        alphas_knotInsertion = zeros(p2 - 1, 1);
        
        % Initialize the Bezier nodes array for the degree elevated Bezier element
        bezierNodes_new = zeros(numNodes1_new * constant_p2pq2_p1 * numNodes3, numDimensions);
        
        % Bezier nodes shared by the next Bezier element
        bezierNodes_nextElement = zeros(numNodes1_new * (p2 - 1) * numNodes3, numDimensions);
    ...
    %----------------------------------------------------------------------
    %  Degree elevation for direction 3
    %----------------------------------------------------------------------
    if (q3 > 0)
        %------------------------------------------------------------------
        %  Shifts to find all the nodes on the same "plane"
        ...
        knots3_new = zeros(numKnots3_new, 1);
        knots3_new((1 : constant_p3pq3_p1)', 1) = xi1;
        index_knot = constant_p3pq3_p1 + 1;
        
        nodes_new = zeros(numNodes12_new * numNodes3_new, numDimensions);
        nodes_new(numNodes12_new + shifts_for_nodes_new, :) = nodes(numNodes12_new + shifts_for_nodes, :);
        index_node = 2;
        
        
        %------------------------------------------------------------------
        %  Set variables for the Bezier element knots1 x knots2 x [xi1, xi2]
        %------------------------------------------------------------------
        % Set the Bezier nodes array to that for the first Bezier element
        bezierNodes = nodes((1 : (numNodes12_new * constant_p3p1))', :);
        
        % Initialize the coefficients for knot insertion
        alphas_knotInsertion = zeros(p3 - 1, 1);
        
        % Initialize the Bezier nodes array for the degree elevated Bezier element
        bezierNodes_new = zeros(numNodes12_new * constant_p3pq3_p1, numDimensions);
        
        % Bezier nodes shared by the next Bezier element
        bezierNodes_nextElement = zeros(numNodes12_new * (p3 - 1), numDimensions);
        ...
        %------------------------------------------------------------------
        % Array of coefficients used to degree elevate the Bezier elements
        alphas_degreeElevation = zeros(constant_p3pq3_p1, constant_p3p1);
        alphas_degreeElevation(1, 1) = 1;
        alphas_degreeElevation(constant_p3pq3_p1, constant_p3p1) = 1;
        
        % Compute the degree elevation coefficients
        for i = 2 : (floor(constant_p3pq3 / 2) + 1)
            temp = 1 / nchoosek(constant_p3pq3, i - 1);
            
            for j = max(1, i - q3) : min(constant_p3p1, i)
                alphas_degreeElevation(i, j) = temp * nchoosek(p3, j - 1) * nchoosek(q3, i - j);
            end
        end
        
        for i = (floor(constant_p3pq3 / 2) + 2) : constant_p3pq3
            for j = max(1, i - q3) : min(constant_p3p1, i)
                alphas_degreeElevation(i, j) = alphas_degreeElevation(constant_p3pq3_p1 - i + 1, constant_p3p1 - j + 1);
            end
        end
        ...
        while (b < numKnots3)
            ...
            while (b < numKnots3 && knots3(b) == knots3(b + 1))
                b = b + 1;
            end
            ...
            % Insert the knot xi2 to get a Bezier element
            numKnotsToInsert = p3 - numRepeats;
            ...
            if (numKnotsToInsert > 0)
                ...
                for i = 1 : numKnotsToInsert
                    alphas_knotInsertion(i) = numerator / (knots3(a + numRepeats + i) - xi1);
                end
                ...
                for i = 1 : numKnotsToInsert
                    ...
                    for k = constant_p3p1 : -1 : (s + 1)
                        alpha = alphas_knotInsertion(k - s);
                        
                        bezierNodes(numNodes12_new * k + shifts_for_bezierNodes, :) = alpha * bezierNodes(numNodes12_new * k + shifts_for_bezierNodes, :) + (1 - alpha) * bezierNodes(numNodes12_new * (k - 1) + shifts_for_bezierNodes, :);
                    end
                    
                    index = numKnotsToInsert - i + 1;
                    bezierNodes_nextElement(numNodes12_new * index + shifts_for_bezierNodes, :) = bezierNodes(numNodes12_new * constant_p3p1 + shifts_for_bezierNodes, :);
                end
                
                rbz = constant_p3pq3_p1 - floor((numKnotsToInsert + 1) / 2);
            else
                rbz = constant_p3pq3_p1;
            end
            ...
            %--------------------------------------------------------------
            % Compute the nodes for the degree elevated Bezier element
            for i = lbz : constant_p3pq3_p1
                bezierNodes_new(numNodes12_new * i + shifts_for_bezierNodes_new, :) = 0;
                
                for j = max(1, i - q3) : min(constant_p3p1, i)
                    bezierNodes_new(numNodes12_new * i + shifts_for_bezierNodes_new, :) = bezierNodes_new(numNodes12_new * i + shifts_for_bezierNodes_new, :) + alphas_degreeElevation(i, j) * bezierNodes(numNodes12_new * j + shifts_for_bezierNodes, :);
                end
            end
            ...
            if (numKnotsToInsert_old > 1)
                ...
                for t = 2 : numKnotsToInsert_old
                    ...
                    while (j - i > t - 1)
                        % Update the nodes
                        if (i < index_node)
                            alpha = (xi2 - knots3_new(i)) / (xi1 - knots3_new(i));
                        ...
                        % Update the nodes for the next Bezier element
                        if (j >= lbz)
                            if (j - t + 1 <= index_knot - constant_p3pq3 + numKnotsToInsert_old)
                                alpha = (xi2 - knots3_new(j - t + 1)) / (xi2 - xi1);
                                
                                bezierNodes_new(numNodes12_new * k + shifts_for_bezierNodes_new, :) = alpha * bezierNodes_new(numNodes12_new * k + shifts_for_bezierNodes_new, :) + (1 - alpha) * bezierNodes_new(numNodes12_new * (k - 1) + shifts_for_bezierNodes_new, :);
                            else
                                alpha = (xi2 - knots3_new(index_knot - 1)) / (xi2 - xi1);
            ...
            %--------------------------------------------------------------
            if (a > constant_p3p1)
                knots3_new(index_knot + (0 : (constant_p3pq3 - numKnotsToInsert_old - 1))') = xi1;
                index_knot = index_knot + (constant_p3pq3 - numKnotsToInsert_old);
            end
            ...
            %--------------------------------------------------------------
            for i = lbz : rbz
                nodes_new(numNodes12_new * index_node + shifts_for_nodes_new, :) = bezierNodes_new(numNodes12_new * i + shifts_for_bezierNodes_new, :);
                index_node = index_node + 1;
            end
            ...
            if (b < numKnots3)
                for i = 1 : numKnotsToInsert
                    bezierNodes(numNodes12_new * i + shifts_for_bezierNodes, :) = bezierNodes_nextElement(numNodes12_new * i + shifts_for_bezierNodes, :);
                end
                
                for i = (numKnotsToInsert + 1) : constant_p3p1
                    bezierNodes(numNodes12_new * i + shifts_for_bezierNodes, :) = nodes(numNodes12_new * (b - constant_p3p1 + i) + shifts_for_nodes, :);
                end
                ...
                % For the last element, insert knot xi2 to the new knot vector
                knots3_new(index_knot + (0 : constant_p3pq3)') = xi2;
                index_knot = index_knot + constant_p3pq3_p1;
            end
        end
        
        p3_new = constant_p3pq3;
