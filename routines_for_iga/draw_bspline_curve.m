...
%  Summary:
%  
%  This routine draws the B-spline curve that is specified by the knot
%  vector of degree p and the nodes (control points) in 1D, 2D, or 3D.
%  
%  
%  Warning:
%  
...
%  
%      draw_bspline_curve(knots, nodes, p);
%  
%  where,
%  
...
%  Output:
%  
%  1. Plot of the B-spline curve
%--------------------------------------------------------------------------
function draw_bspline_curve(knots, nodes, p)
    ...
    for e = 1 : numElements
        ...
        Bspline_der0 = bezierExtractions_e * Bernstein_der0;
        ...
        for j = 1 : numPointsPerElementCoordinate
            ...
            x(j) = nodes_e(1, :) * basis_der0;
            
            if (numDimensions >= 2)
                y(j) = nodes_e(2, :) * basis_der0;
            end
            
            if (numDimensions == 3)
                z(j) = nodes_e(3, :) * basis_der0;
            end
        end
