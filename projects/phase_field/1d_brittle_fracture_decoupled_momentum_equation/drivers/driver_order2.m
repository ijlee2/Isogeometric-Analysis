function driver_order2()
    ...
    for j = 1 : size(numRefinements, 1)
        ...
        path_to_assembly_directory = sprintf('../assembly_files/order2_p%d/numRefinements%d/', i, numRefinements(j));
        
        % Set path to the results directory
        path_to_results_directory = sprintf('../results/order2_p%d/numRefinements%d/', i, numRefinements(j));
