...
%  rectangular bar in tension. We use quadratic B-splines as basis functions.
%  
%  
%  Instructions:
%  
...
function generate_assemblyFile(numRefinements)
    ...
    % Frequency of saves
    numTimeStepsBetweenSaves = 5;
    
    % Parameters for Newton's method
    maxNewtonsMethod = 5;
    tolNewtonsMethod = 1e-8;
    ...
    bar_length = 250e-3;              % Length of the bar
    bar_width = 50e-3;                % Width of the bar
    bar_thickness = 5e-3;             % Thickness of the bar
    
    patches(1).nodes = [0         , 0        , 0            ; ...
                        bar_length, 0        , 0            ; ...
                        0         , bar_width, 0            ; ...
                        bar_length, bar_width, 0            ; ...
                        0         , 0        , bar_thickness; ...
                        bar_length, 0        , bar_thickness; ...
                        0         , bar_width, bar_thickness; ...
    ...
    switch numRefinements
        case 0
            temp = 0.5 * sqrt(setdiff((0 : 0.5^3 : 1)', [0; 1]));
            
            knotsForInsertion1 = [(0 + temp); 0.5; sort(1 - temp)];
            knotsForInsertion2 = 0.5;
            knotsForInsertion3 = 0.5;
            
        case 1
            temp = 0.5 * sqrt(setdiff((0 : 0.5^4 : 1)', [0; 1]));
    ...
    for p = 1 : numPatches
       patches(p).nodes = project_down(patches(p).nodes);
        ...
        %{
        % Draw the B-spline patch (for debugging)
        draw_bspline_volume(patches(p).knots1, patches(p).knots2, patches(p).knots3, patches(p).nodes, patches(p).p1, patches(p).p2, patches(p).p3);
        axis image;
        return;
        %}
    end
