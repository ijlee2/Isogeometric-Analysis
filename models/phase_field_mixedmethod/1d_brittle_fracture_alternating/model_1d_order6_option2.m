...
%--------------------------------------------------------------------------
function model_1d_order6_option2(path_to_assembly_directory, path_to_results_directory, restartTime)
%    addpath('../../../../external_libraries/Factorize/');
    ...
    rows_for_K23    = temp;
    columns_for_K23 = temp;
    values_for_K23  = temp;
    
    rows_for_K32    = temp;
    columns_for_K32 = temp;
    values_for_K32  = temp;
    
    rows_for_K33    = temp;
    columns_for_K33 = temp;
    values_for_K33  = temp;
...
%           f3_e  = elementVector;
            ...
            for q_e = 1 : numQuadraturePointsPerElement
                ...
                if (alternation == (restartTime + 1))
                    ...
                    % The subscripts denote the derivatives of N_A and N_B.
                    C00 = constant_G_c                            ;
                    C11 = constant_G_c * (4/9 * constant_ell_0_sq);
                    ...
                    %------------------------------------------------------
                    %  Form the element matrix K32
                    %------------------------------------------------------
                    % Coefficients for the phase field theory.
                    % The subscripts denote the derivatives of N_A and N_B.
                    C00 = 1                          ;
                    C11 = 8/9   * constant_ell_0_sq  ;
                    C22 = 16/81 * constant_ell_0_sq^2;
                    
                    K32_e = K32_e + (w(q_e) * Jacobian * C00) * (basis_physical_der0 * basis_physical_der0') ...
                                  + (w(q_e) * Jacobian * C11) * (basis_physical_der1 * basis_physical_der1') ...
...
%               f3_e = f3_e + (w(q_e) * Jacobian * C0) * basis_physical_der0;
            ...
            if (alternation == (restartTime + 1))
                ...
                rows_for_K23(index)    = kron(vector_of_ones, index_equation2);
                columns_for_K23(index) = kron(index_equation3, vector_of_ones);
                values_for_K23(index)  = reshape(K23_e, constant_p1p1_sq, 1);
                
                
                % Add the element matrix K32
                rows_for_K32(index)    = kron(vector_of_ones, index_equation3);
                columns_for_K32(index) = kron(index_equation2, vector_of_ones);
                values_for_K32(index)  = reshape(K32_e, constant_p1p1_sq, 1);
                
                
                % Add the element matrix K33
                rows_for_K33(index)    = kron(vector_of_ones, index_equation3);
                columns_for_K33(index) = kron(index_equation3, vector_of_ones);
                values_for_K33(index)  = reshape(K33_e, constant_p1p1_sq, 1);
            end
...
%           f3(index_equation3) = f3(index_equation3) + f3_e;
        ...
        if (alternation == (restartTime + 1))
            K23 = sparse(rows_for_K23, columns_for_K23, values_for_K23, numDOFs_total2, numDOFs_total2);
            K32 = sparse(rows_for_K32, columns_for_K32, values_for_K32, numDOFs_total2, numDOFs_total2);
            K33 = sparse(rows_for_K33, columns_for_K33, values_for_K33, numDOFs_total2, numDOFs_total2);
            
            clear rows_for_K23 columns_for_K23 values_for_K23;
            clear rows_for_K32 columns_for_K32 values_for_K32;
            clear rows_for_K33 columns_for_K33 values_for_K33;
