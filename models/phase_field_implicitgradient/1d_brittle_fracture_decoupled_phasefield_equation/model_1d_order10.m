...
%  tension, so that we can find the minimizing solution to the truncated
%  crack density.
%  
%  
%  Instructions:
...
%  
%      ./matbg.sh driver_order10.m output
%  
%  Note that,
%  
...
%--------------------------------------------------------------------------
function model_1d_order10(path_to_assembly_directory, path_to_results_directory)
    % Feedback for user
    fprintf('\n');
    fprintf('----------------------------------------------------------------\n');
    fprintf('----------------------------------------------------------------\n\n');
    fprintf('  10th-order phase field theory in 1D.\n\n');
    ...
    Bernstein1_der5 = zeros(constant_p1p1, numQuadraturePoints);
    ...
    for q_e = 1 : numQuadraturePoints
        ...
        Bernstein1_der5(:, q_e) = temp(:, 6);
    end
    
    clear z1 temp;
    
    
    % Evaluate the Bernstein polynomials for direction 1 at the left and
    % right endpoints of an element
    Bernstein1_der0_left  = eval_1d_bernstein(0, p1);
    Bernstein1_der0_right = eval_1d_bernstein(1, p1);
    ...
    gamma_1p = gamma(1 / p_exp);
    gamma_3p = gamma(3 / p_exp);
    gamma_5p = gamma(5 / p_exp);
    gamma_7p = gamma(7 / p_exp);
    gamma_9p = gamma(9 / p_exp);
    gamma_11p = gamma(11 / p_exp);
    
    alpha2 = + gamma_3p / (2 * gamma_1p);
    
    alpha4 = - gamma_5p   / (24 * gamma_1p  ) ...
             + gamma_3p^2 / (4  * gamma_1p^2);
    
    alpha6 = + gamma_7p              / (720 * gamma_1p  ) ...
             - gamma_5p   * gamma_3p / (24  * gamma_1p^2) ...
             + gamma_3p^3            / (8   * gamma_1p^3);
    
    alpha8 = - gamma_9p                / (40320 * gamma_1p  ) ...
             + gamma_7p   * gamma_3p   / (720   * gamma_1p^2) ...
             + gamma_5p^2              / (576   * gamma_1p^2) ...
             - gamma_5p   * gamma_3p^2 / (32    * gamma_1p^3) ...
             + gamma_3p^4              / (16    * gamma_1p^4);
    
    alpha10 = + gamma_11p               / (3628800 * gamma_1p  ) ...
              - gamma_9p   * gamma_3p   / (40320   * gamma_1p^2) ...
              - gamma_7p   * gamma_5p   / (8640    * gamma_1p^2) ...
              + gamma_7p   * gamma_3p^2 / (960     * gamma_1p^3) ...
              + gamma_5p^2 * gamma_3p   / (384     * gamma_1p^3) ...
              - gamma_5p   * gamma_3p^3 / (48      * gamma_1p^4) ...
              + gamma_3p^5              / (32      * gamma_1p^5);
    ...
    for e = 1 : numElements1
        ...
        basis_der5 = (dt_dxi^5 * bezierExtractions1_e) * Bernstein1_der5;
        ...
        for q_e = 1 : numQuadraturePointsPerElement
            ...
            basis_physical_der5 = JacobianMatrix_inv^5 * basis_der5(:, q_e);
            ...
            C00 = 1;
            C11 = alpha2 * constant_ell_0_sq;
            C22 = alpha4 * constant_ell_0_sq^2;
            C33 = alpha6 * constant_ell_0_sq^3;
            C44 = alpha8 * constant_ell_0_sq^4;
            C55 = alpha10 * constant_ell_0_sq^5;
            ...
            K22_e = K22_e + (w(q_e) * Jacobian * C00) * (basis_physical_der0 * basis_physical_der0') ...
                          ...
                          + (w(q_e) * Jacobian * C55) * (basis_physical_der5 * basis_physical_der5');
        ...
        basis_der0_left  = bezierExtractions1_e * Bernstein1_der0_left;
        basis_der0_right = bezierExtractions1_e * Bernstein1_der0_right;
        
        x_left  = nodes_e * basis_der0_left;
        x_right = nodes_e * basis_der0_right;
        ...
        if (x_left < 0 && 0 < x_right)
            if (abs(x_left) < 1e-12)
                ...
                columns_for_K(index) = (numDOFs + 1) * ones(constant_p1p1, 1);
                values_for_K(index)  = basis_der0_left;
                ...
                rows_for_K(index)    = (numDOFs + 1) * ones(constant_p1p1, 1);
            ...
            elseif (abs(x_right) < 1e-12)
                ...
                rows_for_K(index)    = index_equation2;
                columns_for_K(index) = (numDOFs + 1) * ones(constant_p1p1, 1);
                values_for_K(index)  = basis_der0_right;
