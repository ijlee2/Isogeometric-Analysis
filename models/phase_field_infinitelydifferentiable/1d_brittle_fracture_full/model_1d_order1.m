...
%  we can validate the 1st-order phase field theory for brittle fracture.
%  
...
%  
%      ./matbg.sh driver_order1.m output
%  
%  Note that,
%  
...
%--------------------------------------------------------------------------
function model_1d_order1(path_to_assembly_directory, path_to_results_directory, restartTime)
    % Feedback for user
    fprintf('\n');
    fprintf('----------------------------------------------------------------\n');
    fprintf('----------------------------------------------------------------\n\n');
    fprintf('  1st-order phase field theory in 1D.\n\n');
    ...
    %----------------------------------------------------------------------
    if (restartTime == 0)
        initialize(1, path_to_assembly_directory, path_to_results_directory);
    ...
    constant_G_c       = 2 * material_G_c * material_A / material_ell_0;
...
%                C10 = 2 * constant_stiffness * c_der0 * u_der1;
                ...
                % Coefficients for the (regular) phase field theory.
                % The subscripts denote the derivatives of N_A and N_B.
%                C00 = constant_stiffness * u_der1^2 + constant_G_c * (minus_log_c^4 - 6 * minus_log_c^3 + 6 * minus_log_c^2);
%                C11 =                                 constant_G_c * constant_ell_0_sq;
                C00 = constant_stiffness * (3 - 6 * c_der0) * u_der1^2 + constant_G_c * (minus_log_c^4 - 6 * minus_log_c^3 + 6 * minus_log_c^2);
                C11 =                                                    constant_G_c * constant_ell_0_sq;
...
%                C1 = constant_stiffness * c_der0^2 * u_der1;
                ...
                % Coefficients for the (regular) phase field theory.
                % The subscript denotes the derivative of N_A.
%                C0 = constant_stiffness * (c_der0 * u_der1^2) + constant_G_c * (c_der0 * (minus_log_c^4 - 2 * minus_log_c^3));
%                C1 =                                            constant_G_c * (c_der1 * constant_ell_0_sq);
                C0 = constant_stiffness * (3 * c_der0 * (1 - c_der0) * u_der1^2) + constant_G_c * (c_der0 * (minus_log_c^4 - 2 * minus_log_c^3));
                C1 =                                                               constant_G_c * (c_der1 * constant_ell_0_sq);
