...
%  we can validate the 3rd-order phase field theory for brittle fracture.
%  
...
%  
%      ./matbg.sh driver_order3.m output
%  
%  Note that,
%  
...
%--------------------------------------------------------------------------
function model_1d_order3(path_to_assembly_directory, path_to_results_directory, restartTime)
    % Feedback for user
    fprintf('\n');
    fprintf('----------------------------------------------------------------\n');
    fprintf('----------------------------------------------------------------\n\n');
    fprintf('  3rd-order phase field theory in 1D.\n\n');
    ...
    %----------------------------------------------------------------------
    if (restartTime == 0)
        initialize(3, path_to_assembly_directory, path_to_results_directory);
    ...
    for k = restartTime : maxNewtonsMethod
        ...
        for e = 1 : numElements1
            ...
            % phase fields
            u1_e = ((1 - constant_HHT_alpha) * u1(index_equation1          ) + constant_HHT_alpha * u_new(index_equation1))';
            u2_e = ((1 - constant_HHT_alpha) * u2(index_equation2 - numDOFs) + constant_HHT_alpha * u_new(index_equation2))';
            ...
            for q_e = 1 : numQuadraturePointsPerElement
                ...
                %----------------------------------------------------------
                %  Evaluate the logarithm of the phase field
                %----------------------------------------------------------
                minus_log_c = -log(max(min(c_der0, 1), eps));
                
                % Evaluate the cubic root of the logarithm
                minus_log_c_curt = nthroot(minus_log_c, 3);
                
                
                if (c_der0 <= 0 || c_der0 > 1)
                    multiplier = 0;
                else
                    multiplier = 1;
                end
...
%                C11 = constant_stiffness * c_der0^2;
                C11 = constant_stiffness * (3 * c_der0^2 - 2 * c_der0^3);
...
%                C10 = 2 * constant_stiffness * c_der0 * u_der1;
                C10 = 6 * constant_stiffness * c_der0 * (1 - c_der0) * u_der1;
                ...
                % Coefficients for the (regular) phase field theory.
                % The subscripts denote the derivatives of N_A and N_B.
%                C00 = constant_stiffness * u_der1^2 + constant_G_c * (9 * minus_log_c_curt^8 - 36 * minus_log_c_curt^5 + 20 * minus_log_c_curt^2);
%                C11 =                                 constant_G_c * constant_ell_0_sq;
                C00 = constant_stiffness * (3 - 6 * c_der0) * u_der1^2 + constant_G_c * (9 * minus_log_c_curt^8 - 36 * minus_log_c_curt^5 + 20 * minus_log_c_curt^2);
                C11 =                                                    constant_G_c * constant_ell_0_sq;
...
%                C1 = constant_stiffness * c_der0^2 * u_der1;
                C1 = constant_stiffness * (3 * c_der0^2 - 2 * c_der0^3) * u_der1;
                ...
                % Coefficients for the (regular) phase field theory.
                % The subscript denotes the derivative of N_A.
%                C0 = constant_stiffness * (c_der0 * u_der1^2) + constant_G_c * (c_der0 * (9 * minus_log_c_curt^8 - 12 * minus_log_c_curt^5));
%                C1 =                                            constant_G_c * (c_der1 * constant_ell_0_sq);
                C0 = constant_stiffness * (3 * c_der0 * (1 - c_der0) * u_der1^2) + constant_G_c * (c_der0 * (9 * minus_log_c_curt^8 - 12 * minus_log_c_curt^5));
                C1 =                                                               constant_G_c * (c_der1 * constant_ell_0_sq);
