...
%  where,
%  
%      order is the order of the phase field theory (1, 2)
...
function initialize(order, path_to_assembly_directory, path_to_results_directory)
    ...
    for e = 1 : numElements1
        ...
        for i = 1 : numNodesToConsider
            u1(LM_array1(offset + i, e)) = function_u_exact(x(i), u_L, material_ell_0, p1);
    ...
    %  Save the fields
    %----------------------------------------------------------------------
    save(sprintf('%sfile_results_iteration%d', path_to_results_directory, 0), 'u1', 'u2', '-v6');
end


function u = function_u_exact(x, u_L, epsilon, p1)
    switch p1
        case 1
            if (x < -epsilon)
                u = 0;
