...
%                C11 = constant_stiffness * c_der0^2;
                C11 = constant_stiffness * (c_der0^2 + u3_e(1) * (material_ell_0 * c_der1)^2 + u3_e(2) * (material_ell_0^2 * c_der2)^2 + u3_e(3) * (material_ell_0^3 * c_der3)^2);
        ...
        for k = 0 : maxNewtonsMethod
            ...
            for e = 1 : numElements1
                ...
                for q_e = 1 : numQuadraturePointsPerElement
                    ...
                    K23_e(:, 1) = K23_e(:, 1) + (w(q_e) * Jacobian * constant1 * c_der1) * basis_physical_der1;
                    K23_e(:, 2) = K23_e(:, 2) + (w(q_e) * Jacobian * constant2 * c_der2) * basis_physical_der2;
                    K23_e(:, 3) = K23_e(:, 3) + (w(q_e) * Jacobian * constant3 * c_der3) * basis_physical_der3;
