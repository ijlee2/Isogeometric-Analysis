...
function model_1d_order4(path_to_assembly_directory, path_to_results_directory)
    ...
    load(sprintf('%sfile_results_iteration%d', path_to_results_directory, 0), 'u1', 'u2', 'lambda0', 'lambda1');
    ...
    for k = 0 : maxNewtonsMethod
        ...
        for e = 1 : numElements1
            ...
            for q_e = 1 : numQuadraturePointsPerElement
                ...
                C11 = C11 + (lambda0(q) * constant0 + lambda1(q) * constant1) + LMM_beta * (3/2 * (constant0^2 + constant1^2) * u_der1^2);
                ...
                % Additional terms from the augmented Lagrangian method
                C1 = C1 + (lambda0(q) * constant0 + lambda1(q) * constant1) + LMM_beta * (1/2 * (constant0^2 + constant1^2) * u_der1);
        ...
        %  Save the results
        %------------------------------------------------------------------
        save(sprintf('%sfile_results_iteration%d', path_to_results_directory, k), 'u1', 'u2', 'lambda0', 'lambda1', '-v6');
    end
