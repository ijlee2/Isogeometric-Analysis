...
function model_1d_order8(path_to_assembly_directory, path_to_results_directory)
    ...
    load(sprintf('%sfile_results_iteration%d', path_to_results_directory, 0), 'u1', 'u2', 'lambda0', 'lambda1', 'lambda2', 'lambda3');
    ...
    for k = 0 : maxNewtonsMethod
        ...
        for e = 1 : numElements1
            ...
            for q_e = 1 : numQuadraturePointsPerElement
                ...
                c_der4 = u2_e * basis_physical_der4;
                ...
                constant2 = constant_stiffness * constant_ell_0_sq^2 * c_der2^2;
                constant3 = constant_stiffness * constant_ell_0_sq^3 * c_der3^2;
                ...
                C11 = C11 + (lambda0(q) * constant0 + lambda1(q) * constant1 + lambda2(q) * constant2 + lambda3(q) * constant3) + LMM_beta * (3/2 * (constant0^2 + constant1^2 + constant2^2 + constant3^2) * u_der1^2);
                ...
                constant2 = constant_stiffness * constant_ell_0_sq^2 * c_der2 * u_der1;
                constant3 = constant_stiffness * constant_ell_0_sq^3 * c_der3 * u_der1;
                ...
                C11 =       lambda1(q) * (2 * constant1) + LMM_beta * (2 * constant1^2 * c_der1 * u_der1);
                C12 =       lambda2(q) * (2 * constant2) + LMM_beta * (2 * constant2^2 * c_der2 * u_der1);
                C13 =       lambda3(q) * (2 * constant3) + LMM_beta * (2 * constant3^2 * c_der3 * u_der1);
                
                
                K12_e = K12_e + w(q_e) * Jacobian * (C10 * (basis_physical_der1 * basis_physical_der0') ...
                                                   + C11 * (basis_physical_der1 * basis_physical_der1') ...
                                                   + C12 * (basis_physical_der1 * basis_physical_der2') ...
                                                   + C13 * (basis_physical_der1 * basis_physical_der3'));
                ...
                constant3 = constant_stiffness * constant_ell_0_sq^3 * u_der1^2;
                ...
                C11 = C11 + lambda1(q) * constant1 + LMM_beta * (3/2 * constant1^2 * c_der1^2);
                C22 = C22 + lambda2(q) * constant2 + LMM_beta * (3/2 * constant2^2 * c_der2^2);
                C33 = C33 + lambda3(q) * constant3 + LMM_beta * (3/2 * constant3^2 * c_der3^2);
                
                
                K22_e = K22_e + w(q_e) * Jacobian * (C00 * (basis_physical_der0 * basis_physical_der0') ...
                                                   + C11 * (basis_physical_der1 * basis_physical_der1') ...
                                                   + C22 * (basis_physical_der2 * basis_physical_der2') ...
                                                   + C33 * (basis_physical_der3 * basis_physical_der3') ...
                                                   + C44 * (basis_physical_der4 * basis_physical_der4'));
                ...
                constant2 = constant_stiffness * constant_ell_0_sq^2 * c_der2^2 * u_der1;
                constant3 = constant_stiffness * constant_ell_0_sq^3 * c_der3^2 * u_der1;
                ...
                % Additional terms from the augmented Lagrangian method
                C1 = C1 + (lambda0(q) * constant0 + lambda1(q) * constant1 + lambda2(q) * constant2 + lambda3(q) * constant3) + LMM_beta * (1/2 * (constant0^2 + constant1^2 + constant2^2 + constant3^2) * u_der1);
                ...
                constant3 = constant_stiffness * constant_ell_0_sq^3 * c_der3 * u_der1^2;
                
                
                % Coefficients for the (regular) phase field theory.
                % The subscript denotes the derivative of N_A.
                C0 = constant0 - constant_G_c * (1 - c_der0);
                C1 = constant_G_c * (        constant_ell_0_sq   * c_der1);
                C2 = constant_G_c * (3/8   * constant_ell_0_sq^2 * c_der2);
                C3 = constant_G_c * (1/16  * constant_ell_0_sq^3 * c_der3);
                C4 = constant_G_c * (1/256 * constant_ell_0_sq^4 * c_der4);
                
                
                % Additional terms from the augmented Lagrangian method
                C0 = C0 + lambda0(q) * constant0 + LMM_beta * (1/2 * constant0^2 * c_der0);
                C1 = C1 + lambda1(q) * constant1 + LMM_beta * (1/2 * constant1^2 * c_der1);
                C2 = C2 + lambda2(q) * constant2 + LMM_beta * (1/2 * constant2^2 * c_der2);
                C3 = C3 + lambda3(q) * constant3 + LMM_beta * (1/2 * constant3^2 * c_der3);
                
                
                f2_e = f2_e + w(q_e) * Jacobian * (C0 * basis_physical_der0 ...
                                                 + C1 * basis_physical_der1 ...
                                                 + C2 * basis_physical_der2 ...
                                                 + C3 * basis_physical_der3 ...
                                                 + C4 * basis_physical_der4);
        ...
        for e = 1 : numElements1
            ...
            for q_e = 1 : numQuadraturePointsPerElement
                ...
                lambda1(q) = lambda1_old(q) + LMM_beta * constant0 * (constant_ell_0_sq   * c_der1^2);
                lambda2(q) = lambda2_old(q) + LMM_beta * constant0 * (constant_ell_0_sq^2 * c_der2^2);
                lambda3(q) = lambda3_old(q) + LMM_beta * constant0 * (constant_ell_0_sq^3 * c_der3^2);
        ...
        %  Save the results
        %------------------------------------------------------------------
        save(sprintf('%sfile_results_iteration%d', path_to_results_directory, k), 'u1', 'u2', 'lambda0', 'lambda1', 'lambda2', 'lambda3', '-v6');
    end
