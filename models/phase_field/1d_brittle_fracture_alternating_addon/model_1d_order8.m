...
%           f1_e  = elementVector;
            ...
            for q_e = 1 : numQuadraturePointsPerElement
                ...
                %----------------------------------------------------------
                degradation_function = c_der0^2 + alpha1 * (material_ell_0 * c_der1)^4 + alpha2 * (material_ell_0^2 * c_der2)^6 + alpha3 * (material_ell_0^3 * c_der3)^8;
...
%           f1(index_equation1) = f1(index_equation1) + f1_e;
        ...
        for k = 0 : maxNewtonsMethod
            ...
            for e = 1 : numElements1
                ...
                for q_e = 1 : numQuadraturePointsPerElement
                    ...
                    constant2 = alpha2 * (15 * constant_stiffness * material_ell_0^12 * c_der2^4 * u_der1^2);
                    constant3 = alpha3 * (28 * constant_stiffness * material_ell_0^24 * c_der3^6 * u_der1^2);
                    
                    
                    % Coefficients for the phase field theory. The subscripts
                    % denote the derivatives of N_A and N_B.
                    C00 = constant0 + constant_G_c;
                    C11 = constant1 + constant_G_c * (        constant_ell_0_sq  );
                    C22 = constant2 + constant_G_c * (3/8   * constant_ell_0_sq^2);
                    C33 = constant3 + constant_G_c * (1/16  * constant_ell_0_sq^3);
                    ...
                    constant2 = alpha2 * (3 * constant_stiffness * material_ell_0^12 * c_der2^5 * u_der1^2);
                    constant3 = alpha3 * (4 * constant_stiffness * material_ell_0^24 * c_der3^7 * u_der1^2);
                    
                    
                    % Coefficients for the phase field theory. The subscript
                    % denotes the derivative of N_A.
                    C0 = constant0 - constant_G_c * (1 - c_der0);
                    C1 = constant1 + constant_G_c * (        constant_ell_0_sq   * c_der1);
                    C2 = constant2 + constant_G_c * (3/8   * constant_ell_0_sq^2 * c_der2);
                    C3 = constant3 + constant_G_c * (1/16  * constant_ell_0_sq^3 * c_der3);
        ...
        % -----------------------------------------------------------------
        %------------------------------------------------------------------
        save(sprintf('%sfile_results_alternation%d', path_to_results_directory, alternation), 'u1', 'u2', 'alpha1', 'alpha2', 'alpha3', '-v6');
    end
