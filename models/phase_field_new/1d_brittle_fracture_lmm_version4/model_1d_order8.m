...
function model_1d_order8(path_to_assembly_directory, path_to_results_directory, restartTime)
    ...
    if (restartTime == 0)
        ...
        load(sprintf('%sfile_results_iteration%d', path_to_results_directory, 0), 'u1', 'u2', 'l0', 'l2');
        
    else
        load(sprintf('%sfile_results_iteration%d', path_to_results_directory, restartTime), 'u1', 'u2', 'l0', 'l2');
    ...
    for k = restartTime : maxNewtonsMethod
        ...
        for e = 1 : numElements1
            ...
            index_equation4 = index_equation3;
            ...
            for q_e = 1 : numQuadraturePointsPerElement
                ...
                %----------------------------------------------------------
                %  Evaluate the Lagrange multiplier fields
                %----------------------------------------------------------
                l0_der0 = l0_e * basis_physical_der0;
                l2_der0 = l2_e * basis_physical_der0;
                ...
                C11 = C11 + (2 * l0_der0 * c_der0^2) + (2 * constant_ell_0_sq^2 * l2_der0 * c_der2^2);
                ...
                % Additional terms from Lagrange multiplier method
                C10 = C10 + (4 * l0_der0 * c_der0 * u_der1);
                C12 =       (4 * constant_ell_0_sq^2 * l2_der0 * c_der2 * u_der1);
                
                K12_e = K12_e + (w(q_e) * Jacobian * C10) * (basis_physical_der1 * basis_physical_der0') ...
                              + (w(q_e) * Jacobian * C12) * (basis_physical_der1 * basis_physical_der2');
                ...
                %----------------------------------------------------------
                % Additional terms from Lagrange multiplier method
                C10 = 2 * c_der0^2 * u_der1;
                
                K13_e = K13_e + (w(q_e) * Jacobian * C10) * (basis_physical_der1 * basis_physical_der0');
                
                
                %----------------------------------------------------------
                %  Form the element matrix K14
                %----------------------------------------------------------
                % Additional terms from Lagrange multiplier method
                C10 = 2 * constant_ell_0_sq^2 * c_der2^2 * u_der1;
                
                K14_e = K14_e + (w(q_e) * Jacobian * C10) * (basis_physical_der1 * basis_physical_der0');
                ...
                % Additional terms from Lagrange multiplier method
                C00 = C00 + (2 * l0_der0 * u_der1^2);
                C22 = C22 + (2 * constant_ell_0_sq^2 * l2_der0 * u_der1^2);
                ...
                %----------------------------------------------------------
                % Additional terms from Lagrange multiplier method
                C00 = 2 * c_der0 * u_der1^2;
                
                K23_e = K23_e + (w(q_e) * Jacobian * C00) * (basis_physical_der0 * basis_physical_der0');
                
                
                %----------------------------------------------------------
                %  Form the element matrix K24
                %----------------------------------------------------------
                % Additional terms from Lagrange multiplier method
                C20 = 2 * constant_ell_0_sq^2 * c_der2 * u_der1^2;
                
                K24_e = K24_e + (w(q_e) * Jacobian * C20) * (basis_physical_der2 * basis_physical_der0');
                ...
                % Additional terms from Lagrange multiplier method
                C1 = C1 + (2 * l0_der0 * c_der0^2 * u_der1) + (2 * constant_ell_0_sq^2 * l2_der0 * c_der2^2 * u_der1);
                ...
                % Additional terms from Lagrange multiplier method
                C0 = C0 + (2 * l0_der0 * c_der0 * u_der1^2);
                C2 = C2 + (2 * constant_ell_0_sq^2 * l2_der0 * c_der2 * u_der1^2);
                ...
                f2_e = f2_e + (w(q_e) * Jacobian * C0) * basis_physical_der0 ...
                            ...
                            + (w(q_e) * Jacobian * C4) * basis_physical_der4;
                ...
                %----------------------------------------------------------
                % Additional terms from Lagrange multiplier method
                C0 = c_der0^2 * u_der1^2;
                
                f3_e = f3_e + (w(q_e) * Jacobian * C0) * basis_physical_der0;
                
                
                %----------------------------------------------------------
                %  Form the element RHS vector f4
                %----------------------------------------------------------
                % Additional terms from Lagrange multiplier method
                C0 = constant_ell_0_sq^2 * c_der2^2 * u_der1^2;
                
                f4_e = f4_e + (w(q_e) * Jacobian * C0) * basis_physical_der0;
...
%            K14_e = elementMatrix;
            ...
            index_equation4 = index_equation4 + 3 * numDOFs;
            ...
            % Add the element matrix K14
            index = lastIndex_for_K + indices_for_elementMatrix;
            
            rows_for_K(index)    = kron(vector_of_ones, index_equation1);
            columns_for_K(index) = kron(index_equation4, vector_of_ones);
            values_for_K(index)  = reshape(K14_e, constant_p1p1_sq, 1);
            ...
            % Add the element matrix K24
            index = lastIndex_for_K + indices_for_elementMatrix;
            
            rows_for_K(index)    = kron(vector_of_ones, index_equation2);
            columns_for_K(index) = kron(index_equation4, vector_of_ones);
            values_for_K(index)  = reshape(K24_e, constant_p1p1_sq, 1);
            ...
            % Add the element matrix K34
            % Do nothing (K34 = 0)
            
            
            % Add the element matrix K41
            index = lastIndex_for_K + indices_for_elementMatrix;
            
            rows_for_K(index)    = kron(vector_of_ones, index_equation4);
            columns_for_K(index) = kron(index_equation1, vector_of_ones);
            values_for_K(index)  = reshape(K14_e', constant_p1p1_sq, 1);
            ...
            % Add the element matrix K43
            % Do nothing (K43 = 0)
            
            
            % Add the element matrix K44
            % Do nothing (K44 = 0)
            ...
            % Add the element RHS vector f4
            f(index_equation4) = f(index_equation4) + f4_e;
        ...
        if (k > 0)
            ...
            save(sprintf('%sfile_results_iteration%d', path_to_results_directory, k), 'u1', 'u2', 'l0', 'l2', '-v6');
...
%        clear precond;
        ...
        l2 = l2 + u_increment((1 : numDOFs)' + 3 * numDOFs);
