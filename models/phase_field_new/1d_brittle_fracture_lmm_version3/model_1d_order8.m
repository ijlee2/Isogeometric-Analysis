...
%           f4_e  = elementVector;
            ...
            index_equation4 = index_equation3 + numDOFs;
            ...
            for q_e = 1 : numQuadraturePointsPerElement
                ...
                %----------------------------------------------------------
                %  Form the element matrix K23, K24
                %----------------------------------------------------------
            ...
            % Discard entries in K24 that are deemed too small
            rows = kron(vector_of_ones, index_equation2);
            cols = kron(index_equation4, vector_of_ones);
            vals = reshape(K24_e, constant_p1p1_sq, 1);
            ...
            % Discard entries in K42 that are deemed too small
            rows = kron(vector_of_ones, index_equation4);
            cols = kron(index_equation2, vector_of_ones);
            vals = reshape(K24_e', constant_p1p1_sq, 1);
...
%        clear precond;
        ...
        % -----------------------------------------------------------------
        %------------------------------------------------------------------
        save(sprintf('%sfile_results_alternation%d', path_to_results_directory, alternation), 'u1', 'u2', 'l0', 'l2', '-v6');
    end
