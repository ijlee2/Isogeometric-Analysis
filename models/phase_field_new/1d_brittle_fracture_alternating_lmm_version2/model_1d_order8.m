...
function model_1d_order8(path_to_assembly_directory, path_to_results_directory, restartTime)
    ...
    temp = zeros(constant_p1p1, numQuadraturePoints);
    ...
    index_equation3 = (numDOFs + 1 : numDOFs + numConstraints)';
    index_u2 = [index_u2; index_equation3];
    ...
    % Initialization for element matrices K11, K22
    elementMatrix1 = zeros(constant_p1p1);
    
    % Initialization for element matrix K23
...
%    numMatrixEntriesPerElement2 = constant_p1p1_sq + 2 * numConstraints * constant_p1p1;
...
%        clear precond;
        ...
        for e = 1 : numElements1
            ...
            for q_e = 1 : numQuadraturePointsPerElement
                ...
                %----------------------------------------------------------
                % Coefficients for the (regular) phase field theory.
                % The subscript denotes the derivatives of N_A.
                C0 = u_der1^2;
                C1 = material_ell_0 * C0;
                C2 = constant_ell_0_sq * C0;
                C3 = constant_ell_0_sq * C1;
                
                K23_e(:, 1) = K23_e(:, 1) + (w(q_e) * Jacobian * C0) * basis_physical_der0;
                K23_e(:, 2) = K23_e(:, 2) + (w(q_e) * Jacobian * C1) * basis_physical_der1;
                K23_e(:, 3) = K23_e(:, 3) + (w(q_e) * Jacobian * C2) * basis_physical_der2;
                K23_e(:, 4) = K23_e(:, 4) + (w(q_e) * Jacobian * C3) * basis_physical_der3;
            ...
            rows = kron(vector_of_ones2, index_equation2);
            cols = kron(index_equation3, vector_of_ones1);
            vals = reshape(K23_e, constant_p1p1_x_numConstraints, 1);
            ...
            rows_for_K22(index)    = rows(indices_for_valuesKept);
            columns_for_K22(index) = cols(indices_for_valuesKept);
            values_for_K22(index)  = valuesKept;
            
            lastIndex_for_K = lastIndex_for_K + number_valuesKept;
            
            
            % Discard entries in K32 that are deemed too small
            rows = kron(vector_of_ones1, index_equation3);
            cols = kron(index_equation2, vector_of_ones2);
            vals = reshape(K23_e', constant_p1p1_x_numConstraints, 1);
            ...
            % Add the element RHS vector f3 (= 0)
            % Do nothing
        ...
        K22 = sparse(rows_for_K22(indices_for_valuesKept), columns_for_K22(indices_for_valuesKept), values_for_K22(indices_for_valuesKept), numDOFs_total2, numDOFs_total2);
...
%        clear precond;
    ...
    clear u1 u2;
    clear rows_for_K11 columns_for_K11 values_for_K11;
    clear rows_for_K22 columns_for_K22 values_for_K22;
