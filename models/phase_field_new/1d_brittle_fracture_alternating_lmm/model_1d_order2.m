...
function model_1d_order2(path_to_assembly_directory, path_to_results_directory, restartTime)
    ...
    temp2 = zeros(numMatrixEntriesPerElement2 * numElements1 + 2 * numConstraints * constant_p1p1, 1);
        ...
        for e = 1 : numElements1
            ...
            if (x_left <= 0 && 0 < x_right)
                if (abs(x_left) < 1e-12)
                    ...
                    rows_for_K22(index)    = index_equation2;
                    columns_for_K22(index) = (numDOFs + 1) * ones(constant_p1p1, 1);
                    values_for_K22(index)  = basis_der0(:, numQuadraturePoints + 1);
                    ...
                    rows_for_K22(index)    = (numDOFs + 1) * ones(constant_p1p1, 1);
                    columns_for_K22(index) = index_equation2;
                    values_for_K22(index)  = basis_der0(:, numQuadraturePoints + 1);
                ...
                elseif (abs(x_right) < 1e-12)
                    ...
                    rows_for_K22(index)    = index_equation2;
                    columns_for_K22(index) = (numDOFs + 1) * ones(constant_p1p1, 1);
                    values_for_K22(index)  = basis_der0(:, numQuadraturePoints + 2);
